    ...
    for _, row in rows.iterrows():
        ...
        for keyword in keywords:
            if keyword.lower() in subject.lower():
                return row['keyword_path'], row['special_case'], row['link_download'], True

    for _, row in rows.iterrows():
        if pd.notna(row['coper_name']) and row['coper_name'].lower() in subject.lower():
            return row['save_path'], row['special_case'], row['link_download'], False

    if not rows.empty:
        return rows.iloc[0]['save_path'], rows.iloc[0]['special_case'], rows.iloc[0]['link_download'], False
...
def download_report_from_link(email_body, save_path):
    link_pattern = re.compile(r'(https?://[^\s]+)')
    match = link_pattern.search(email_body)
    if match:
        url = match.group(0)
        ...
        if response.status_code == 200:
            ...
            if content_disposition:
                ...
                if filename:
                    filename = sanitize_filename(filename[0])
                else:
...
def process_email(item, sender_path_table, default_year, specific_date_str):
    ...
    while retries > 0 and not processed:
        try:
            ...
            if not year:
                year = default_year
            base_path, special_case, link_download, is_keyword_path = find_path_for_sender(sender_email, item.Subject, sender_path_table)
            if base_path:
            ...
            if link_download.lower() == 'yes':
                filename = download_report_from_link(item.Body, save_path)
                if filename:
                    logs.append(f"Downloaded: {filename} from link in email body to {save_path}")
                else:
                    logs.append(f"Failed to download report from link in email body for email with subject '{item.Subject}'")
            else:
